...
public class BiomeConfig extends ConfigFile
{
    ...
    protected void writeConfigSettings(SettingsWriter writer) throws IOException
    {
        ...
        if(worldConfig.biomeMode == TerrainControl.getBiomeModeManager().BEFORE_GROUPS)
        {
	        // Note to PG: this is the old (1.7.10) code
	        // The new code seems to cause problems when used with old (pre biome-group) configs.
	        // It seems to work fine at first but causes problems when the server is restarted.
	        // Problems occur along the world border where generation is resumed. Cut off terrain and biomes, vertical walls etc.
